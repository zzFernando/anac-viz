...
function AirportCombobox({ airports, value, selectedLabel, onChange }: AirportComboboxProps) {
  ...
  return (
    <div ref={rootRef} className="relative w-full min-w-[220px] max-w-[460px]">
      <div className={`flex h-8 items-center gap-2 rounded-md border bg-white px-2 text-sm transition-colors
        ${open ? "border-anac-light ring-2 ring-anac-light/10" : "border-gray-200"}`}
      >
        ...
        <input
          ...
          className="min-w-0 flex-1 bg-transparent text-sm text-slate-800 placeholder:text-slate-600 focus:outline-none"
          role="combobox"
          aria-expanded={open}
      ...
      {open && (
        <div
          id="airport-results"
          role="listbox"
          className="absolute left-0 right-0 top-9 z-50 max-h-80 overflow-auto rounded-md border border-gray-200 bg-white py-1 shadow-lift"
        >
            ...
            return (
              <button
                ...
                className={`grid w-full grid-cols-[4.2rem_2.2rem_minmax(0,1fr)_1.4rem] items-center gap-2 px-3 py-2 text-left text-sm
                  ...
                  ${selected ? "text-anac-blue" : "text-slate-700"}`}
              >
                <span className="font-mono text-[0.8rem] font-bold tracking-wide">{display.code}</span>
                <span className="text-[0.72rem] font-bold uppercase tracking-wide text-slate-500">{display.uf || "—"}</span>
                <span className="min-w-0">
                  <span className="block truncate font-medium">{display.cidade || "—"}</span>
                  <span className="block truncate text-xs text-slate-500">{display.nome || "—"}</span>
                </span>
                {selected && <Check size={15} className="text-anac-blue" aria-hidden="true" />}
              </button>
            );
          }) : (
            <div className="px-3 py-3 text-sm text-slate-500">Nenhum aeroporto encontrado</div>
          )}
        </div>
      )}
    </div>
