...
export default function AdsPorSistemaLollipop({ data }: { data: AdsResumo }) {
  ...
  useEffect(() => {
    ...
    g.selectAll("title.tip")
      .data(items)
      .enter().append("title")
      .text(d => `${d.full}\n${d.n.toLocaleString("pt-BR")} regras vigentes`);
    ...
    g.append("g")
      ...
      .call(s => s.selectAll("text")
        .attr("font-size", 8).attr("fill", UI.labelDark).attr("dx", -4))
      .selectAll("text")
      .append("title")
